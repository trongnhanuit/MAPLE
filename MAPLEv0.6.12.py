...
#TODO function to update nDesc when changing a branch length in the tree
...
def findBestParentTopology(tree,node,child,bestLKdiff,removedBLen,strictTopologyStopRules=strictTopologyStopRules,allowedFailsTopology=allowedFailsTopology,thresholdLogLKtopology=thresholdLogLKtopology,sequentialSearch=True):
	...
	while nodesToVisit:
		...
		if direction==0:
			...
			if dist[t1]>effectivelyNon0BLen and (not (up[t1]==node or up[t1]==None)):
				if needsUpdating:
			...
			midBottom=None
			if dist[t1]>effectivelyNon0BLen and up[t1]!=None: #try appending mid-branch
				if needsUpdating:
					otherProbVect=probVect[otherChild]
					if mutations[otherChild]:
						otherProbVect=passGenomeListThroughBranch(otherProbVect,mutations[otherChild],dirIsUp=True)
...
def findBestParentForNewSample(tree,root,diffs,sample,computePlacementSupportOnly):
	...
	if not children[root]: #check if the new leaf is strictly less informative than already placed leaf
		...
		if comparison==1 and not computePlacementSupportOnly:
			...
			#if HnZ:
			#	print("checking nDesc0 after placing minor sequence near the root")
			#	calculateNDesc0(tree,root,checkExisting=True)
	...
	while nodesToVisit:
		...
		if not children[t1]: #check if the new leaf is strictly less informative than already placed leaf
			...
			if comparison==1 and not computePlacementSupportOnly:
				...
				#TODO remove after testing
				#if HnZ:
				#	print("checking nDesc0 after placing a minor sequence")
				#	calculateNDesc0(tree,root,checkExisting=True)
				return t1, 1.0, None, diffs
			elif comparison==2:
				totalMissedMinors[0]+=1

		if dist[t1]>effectivelyNon0BLen and up[t1]!=None: # try first placing as a descendant of the mid-branch point of the branch above the current node.
...
def placeSampleOnTree(tree,node,newPartials,sample,newChildLK, bestUpLength, bestDownLength, bestAppendingLength,pseudoMutCounts):
	...
	if tryNewRoot:
		...
		#TODO remove after testing
		#if HnZ:
		#	print("checking nDesc0 after placing a tip near the root")
		#	calculateNDesc0(tree,newRoot,checkExisting=True)
		return newRoot
	...
	#TODO remove after testing
	#if HnZ:
	#	print("checking nDesc0 after placing a normal tip")
	#	while up[node]!=None:
	#		node=up[node]
	#	calculateNDesc0(tree,node,checkExisting=True)
...
def placeSubtreeOnTree(tree,node,newPartials,appendedNode,newChildLK,bestBranchLengths): #,parentNode,parentNodeReplacements=1
	...
	if tryNewRoot:
		...
		updatePartials(tree,nodeList)
		#if HnZ:# TODO remove after debugging is done
		#	print("check nDesc0 correctness after moving a subtree to new root position")
		#	calculateNDesc0(tree,newRoot,checkExisting=True)
		return newRoot
	...
	updatePartials(tree,nodeList)
	#if HnZ:# TODO remove after debugging is done
	#	print("check nDesc0 correctness after moving a subtree")
	#	root=node
	#	while up[root]!=None:
	#		root=up[root]
	#	calculateNDesc0(tree,root,checkExisting=True) 
...
def cutAndPasteNode(tree,node,bestNode,bestBranchLengths,bestLK,passedProbVect):
	...
	if up[sibling]==None:
		...
		updatePartials(tree,nodeList)
	#if HnZ:# TODO remove after debugging is done
	#	print("check nDesc0 correctness after removing a subtree")
	#	root=sibling
	#	while up[root]!=None:
	#		root=up[root]
	#	calculateNDesc0(tree,root,checkExisting=True) 
	...
	if up[node]!=None:
		...
		if (not topologyUpdated) and bLenChanged:
			...
			if totalImprovement==float("inf"):
				...
				raise Exception("exit")
			#if HnZ:# TODO remove after debugging is done
			#	print("check nDesc0 correctness after branch length change")
			#	root=node
			#	while up[root]!=None:
			#		root=up[root]
			#	calculateNDesc0(tree,root,checkExisting=True) 
